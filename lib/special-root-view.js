...
class SpecialRootEntry {
  constructor (filePath, entryClassName) {
    ...
    this.element.classList.add(entryClassName, 'tree-view-special-entry', this.isDirectory ? 'directory' : 'file', 'entry', 'list-item')
...
class SpecialRootSection {
  constructor (config) {
    ...
    this.element.classList.add(config.className, 'tree-view-special', 'list-tree', 'has-collapsable-children')

    // Header entry (looks like a project root)
    this.headerEntry = document.createElement('li')
    this.headerEntry.classList.add(`${config.className}-root`, 'tree-view-special-root', 'directory', 'entry', 'list-nested-item', 'project-root')
