...
class TreeView {
  ...
  entryClicked(event) {
    ...
    if (entry.classList.contains('directory')) {
      let openOnSingle = atom.config.get('tree-view-plus.openFoldersOnSingleClick');
      if (detail === 1) {
        this.selectEntry(entry);
        // Arrow area always responds to single click
        if (openOnSingle || event.offsetX <= 10) {
          entry.toggleExpansion(isRecursive);
        }
      } else if (detail === 2 && !openOnSingle) {
        entry.toggleExpansion(isRecursive);
      }
    } else if (entry.classList.contains('file')) {
      let openOnSingle = atom.config.get('tree-view-plus.openFilesOnSingleClick');
      if (detail === 1) {
        this.selectEntry(entry);
        if (openOnSingle) {
        ...
        if (openOnSingle) {
          // detail=2 with single-click mode: open permanently (existing behavior)
          return this.fileViewEntryClicked(event);
        } else {
          // detail=2 with double-click mode: open the file
